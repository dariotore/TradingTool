...
const ACTION_CFG: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  STRONG_BUY:  { label: "Forte Acquisto", color: "text-emerald-300", bg: "bg-emerald-500/15 border-emerald-400/30", dot: "bg-emerald-300" },
  ...
  STRONG_SELL: { label: "Forte Vendita",  color: "text-red-300",     bg: "bg-red-500/15 border-red-400/30",         dot: "bg-red-300"     },
};
    ...
    <div className="bg-[#0e1b2e] border border-[#1a2e48] rounded-xl overflow-hidden">
      ...
      {items.length === 0 ? (
        <div className="h-[150px] flex flex-col items-center justify-center gap-2 pb-4">
          ...
          <p className="text-[9px] text-[var(--text-3)]">I dati appariranno dopo i primi cicli di analisi</p>
        </div>
      ) : (
        <div className="px-2 pb-4">
          <ResponsiveContainer width="100%" height={150}>
            <BarChart data={items} barSize={14} margin={{ top: 4, right: 4, bottom: 0, left: -22 }}>
              ...
              <Bar dataKey="correct" stackId="a" name="Corretti" fill="#34d399" />
              <Bar dataKey="wrong"   stackId="a" name="Errati"   fill="#f87171" />
              <Bar dataKey="pending" stackId="a" name="In attesa" fill="#334155" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
          {/* Legend */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--text-3)] mt-0.5">
              {signals.length} segnali · {evaluated.length} valutati
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            title="Elimina segnali senza SL/TP"
      ...
      <div className="flex-1 overflow-auto px-4 py-5 max-w-7xl mx-auto w-full flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard
            ...
            accent="border-[#1a2e48]"
          />
          <StatCard
            label="Accuratezza 24h"
            value={
              accuracy != null
                ? <span className={accuracy >= 60 ? "text-emerald-400" : accuracy >= 50 ? "text-amber-400" : "text-red-400"}>
                    {accuracy}%
                  </span>
                : <span className="text-slate-500 text-base">—</span>
            }
            sub={evaluated.length > 0 ? `${correct} corretti su ${evaluated.length}` : "dati insufficienti (min. 10)"}
            icon={<Target size={16} />}
        ...
        <div className="bg-[#0e1b2e] border border-[#1a2e48] rounded-xl overflow-hidden">
          ...
          {/* Table toolbar */}
          <div className="flex flex-col gap-2 px-4 py-3 border-b border-[#1a2e48]">
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1 max-w-xs">
                ...
                <input
                  ...
                  className="w-full bg-[#0a1525] border border-[#1a2e48] text-[var(--text-2)] text-xs rounded-lg pl-7 pr-3 py-1.5 focus:outline-none focus:border-blue-500/50 transition-colors placeholder:text-[var(--text-3)]"
                />
                {search && (
                  <button
                    onClick={() => setSearch("")}
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-[var(--text-3)] hover:text-white"
                  >
                    ×
                  </button>
                )}
                  ...
                  <button
                    ...
                    className={`text-[10px] font-semibold px-2 py-1 rounded-md border transition-all ${
                      pageSize === n
                        ...
                        : "text-[var(--text-3)] border-[#1a2e48] hover:text-white"
                    }`}
                  >
                    {n}
                  </button>
            ...
            {/* Row 2: market + action filters */}
            <div className="flex items-center gap-2 flex-wrap">
              <SlidersHorizontal size={11} className="text-[var(--text-3)] shrink-0" />
              {["all", "crypto", "forex"].map(m => (
                <button
                  ...
                  className={`text-[10px] font-semibold px-2.5 py-1 rounded-md border transition-all ${
                    market === m
                      ? "bg-blue-500/15 text-blue-400 border-blue-500/30"
                      : "text-[var(--text-3)] border-transparent hover:border-[#1a2e48] hover:text-[var(--text-2)]"
                  }`}
                >
              ...
              {(search || market !== "all" || action !== "all" || dateRange !== "all") && (
                <button
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-[var(--text-3)] mt-1 max-w-xs">
                  {market !== "all" || action !== "all"
                    ...
                    : "I segnali BUY e SELL vengono salvati automaticamente ogni ciclo di analisi (ogni 60 s)."}
                </p>
              </div>
            </div>
          ) : (
              ...
              <table className="w-full border-collapse text-xs">
                <thead className="sticky top-0 z-10 border-b border-[#1a2e48] bg-[#080f1e]">
                  <tr>
                    ...
                    <th className="px-3 py-3 text-left text-[10px] font-semibold text-[var(--text-3)] uppercase tracking-wide hidden md:table-cell">Prezzo</th>
                    ...
                    <th className="px-3 py-3 text-left text-[10px] font-semibold text-[var(--text-3)] uppercase tracking-wide hidden sm:table-cell">4h</th>
                ...
                <tbody>
                  {paginated.map(s => {
                    ...
                    return (
                      <tr
                        key={s.id}
                        className={`border-b border-[#0d1829] transition-colors text-xs ${rowStyle(s)}`}
                      >
                        {/* Date */}
                        ...
                        {/* Price */}
                        <td className="px-3 py-3 tabular font-mono text-[var(--text-2)] whitespace-nowrap hidden md:table-cell">
                        ...
                        <td className="px-3 py-3 hidden lg:table-cell">
                          {s.stop_loss != null ? (
                            <div className="flex flex-col gap-0.5">
                              <span className="text-[10px] text-red-400 font-mono tabular">
                                ↓ {fmtPrice(s.stop_loss)}
                              </span>
                              {s.take_profit != null && (
                                <span className="text-[10px] text-emerald-400 font-mono tabular">
          ...
          {!loading && filtered.length > 0 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-[#1a2e48] flex-wrap gap-2">
              {/* Counter */}
              <span className="text-[10px] text-[var(--text-3)]">
                Segnali <span className="text-[var(--text-2)] font-semibold tabular">{pageStart}–{pageEnd}</span> di{" "}
                <span className="text-[var(--text-2)] font-semibold tabular">{filtered.length}</span>
                {filtered.length !== signals.length && (
                  <span className="text-amber-400/70"> (filtrati da {signals.length})</span>
                )}
              </span>
              ...
              {totalPages > 1 && (
                <div className="flex items-center gap-1">
                  ...
                  {pageNumbers().map((n, i) =>
                    n === "…" ? (
                      <span key={`ellipsis-${i}`} className="px-1.5 text-[10px] text-[var(--text-3)]">…</span>
                    ) : (
                      <button
                        key={n}
                        ...
                        className={`min-w-[28px] h-7 px-1.5 rounded-md text-[10px] font-semibold border transition-all ${
                          page === n
                            ...
                            : "text-[var(--text-3)] border-transparent hover:border-[#1a2e48] hover:text-white"
                        }`}
                      >
                        {n}
                      </button>
