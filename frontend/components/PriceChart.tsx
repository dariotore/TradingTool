...
export default function PriceChart({
  ...
  useEffect(() => {
    ...
    if (source === "forex") {
      ...
      url = `https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=${tf.binanceInterval}&limit=${tf.limit}`;
    }
    ...
    fetch(url)
      ...
      .then((raw: unknown) => {
        ...
        if (source === "forex" || source === "commodity") {
          ...
          setData((raw as unknown[][]).map(k => ({ ts: k[0] as number, c: parseFloat(k[4] as string) })));
        }
      })
      .catch(() => {})
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
              ...
              <Tooltip
                ...
                labelFormatter={(ts: number) => tf.formatTooltip(ts)}
                formatter={(v: number) => [fmtPrice(v), "Prezzo"]}
